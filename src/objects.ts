...
export function isValid(question: Question, answer: string): boolean {
    if (question.type === "short_answer_question") return answer ? true : false;
...
export function toMarkdown(question: Question): string {
    let firstPartNoOptions = `# ${question.name}\n${question.body}`;
    if (question.type === "short_answer_question") return firstPartNoOptions;
    let optionsPart = "\n";
    for (let i = 0; i < question.options.length - 1; i++)
        optionsPart = optionsPart.concat(`- ${question.options[i]}\n`);
    return firstPartNoOptions.concat(
        optionsPart,
