...
export function findQuestion(
    ...
    return foundQuestion.length > 0 ? foundQuestion[0] : null;
}
...
export function toCSV(questions: Question[]): string {
    let firstLine: string = `id,name,options,points,published`;
    let optionStrings: string[] = questions.map(
        (question: Question): string =>
            `\n${question.id},${question.name},${question.options.length},${question.points},${question.published}`,
    );
    return firstLine.concat(...optionStrings);
}
...
export function sameType(questions: Question[]): boolean {
    let firstType = questions[0];
    return questions.every(
        (question: Question): boolean => question.type === firstType.type,
    );
}
...
export function duplicateQuestionInArray(
    ...
    let dupQ = questions.filter(
        (question: Question): boolean => question.id === targetId,
    );
    let dupQ2 = duplicateQuestion(newId, dupQ[0]);
    return [
        ...questions.slice(
            0,
            questions.findIndex(
